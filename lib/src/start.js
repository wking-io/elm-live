...
const { pipe, getPropOr } = require('crocks/helpers')
...
const ws = require('ws')
const { update, readFile, notExists, writeFile, isObj, empty } = require('./utils')
...
function maybeOpen (model) {
  if (model.open && empty(model.server.wss.clients)) {
...
const start = model => model.useServer ? pipe(
  when(getPropOr(false, 'ssl'), getSsl),
