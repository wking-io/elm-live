...
/**
 * isRoot :: { pathname: String, dir: String, pushstate: Bool } -> Bool
 *
 * This function returns boolean if the variables passed indicate the root.
 **/
const isRoot = ({ pathname, dir, pushstate }) =>
  ((pushstate && (!existsSync(dir + pathname.replace(/\?.*$/, '')))) ||
    pathname === '/' ||
    pathname === '')

/**
 * getRequestType :: ({ pathname: String, dir: String }, { proxyPrefix: String, pushstate: Bool }) -> String
 *
 ...
 **/
function getRequestType ({ pathname, dir }, { proxyPrefix, pushstate }) {
  if (pathname === '/reload/reload.js') {
    return RELOAD_TYPE
  } else if (pathname.startsWith(proxyPrefix)) {
    return PROXY_TYPE
  } else if (
    isRoot({ pathname, pushstate, dir })
  ) {
    return CONTENT_TYPE
  } else {
    return STATIC_TYPE
...
function parseUrl (pathname, model) {
  const requestType = getRequestType({ pathname, dir: model.dir }, model)
  const rootPath = `${model.dir}/${model.startPage}`
  const fullPath = `${model.dir}${pathname}`
  return {
    rootPath,
    getPath: isRoot({ pathname, pushstate: model.pushstate, dir: model.dir }) ? rootPath : fullPath,
    isRoot: isRoot({ pathname, pushstate: model.pushstate, dir: model.dir }),
