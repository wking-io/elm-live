...
(function refresh () {
  const verboseLogging = '{{verbose}}' // This is dynamically update by elm-reload.js file before it is sent to the browser
  ...
  */
  ...
  const htmlHeader = (title, path, cwd, ide, start, msg) => `<span style="${parseStyle({ color: 'blue' })}">${parseHeader(title, path)} [<a style="color: inherit" href="${ideUrl(path, ide, cwd, start)}">open in ${capitalizeFirstLetter(ide)}</a>]</span>\n\n`
...
>
  ...
  <pre
    ...
    style="
      ...
      margin: 0;
      padding: 30px;
      font-family: 'Dank Mono', 'Fira Code', Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier ;New', monospace;
    "
  >${error}</pre>
</div>
`
