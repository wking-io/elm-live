...
const { map, chain, equals } = require('crocks/pointfree')
const { pipe } = require('crocks/helpers')
const { ifElse, not } = require('crocks/logic')
const spawn = require('cross-spawn')
const { inject } = require('elm-hot')
const { PROCESS_SUCCESS, update, readFile, writeFile, always } = require('./utils')
...
const buildIsReload = model => model.build.action === model.getAction('reload')
const updateToWatching = model => update(model, { build: { action: model.getAction('watch') } })
...
const run = key => model => Async((reject, resolve) => {
  ...
  const maybeReject = model.recover ? always(resolve(model)) : reject

  spawn(model[key], [], { stdio: [model.input, model.output, model.output] })
    .on('error', maybeReject)
    .on('exit', ifElse(not(equals(PROCESS_SUCCESS)), maybeReject, always(resolve(model))))
})
...
const loadMessage = model => {
  if (model.build.action === model.getAction('buildFailure')) {
    update(model.build, { ide: model.ide, cwd: model.cwd })
