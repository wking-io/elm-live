...
const serverStarted = config =>
  `
Reload web server:
  - Website URL: ${chalk.blue.bold((config.ssl ? 'https://' : 'http://') + (config.host || 'localhost') + ':' + (config.port || 1234))}
  - Listening on port: ${chalk.cyan.bold(config.port || 1234)}
  - Monitoring dir: ${chalk.green.bold(config.dir || process.cwd())}
${config.proxyPrefix && config.proxyHost
    ? `- Proxying requests starting with ${chalk.green(config.proxyPrefix)} to ${chalk.green(config.proxyHost)}`
    : ''}
`
